...
npm run server        # WebSocket server (tsx src/server/ws-server.ts), port from WS_PORT (default 3001)
npm run build         # tsc compile to dist/
npm start             # run compiled dist/cli/repl.js
npm run dev:all        # runs server + ui concurrently
...
```
npm run ui             # vite dev server (port 5173, proxies /ws to localhost:3001)
...
**Agent core** (`src/agent/core.ts`) is the single loop that all surfaces (CLI, voice, WebSocket) go
...
**WebSocket server** (`src/server/ws-server.ts`) is the transport for the `ui/` frontend — this is
where the pieces above get wired together per-connection: one `Agent` + `ToolRegistry` per client
socket, conversation history persisted as JSON per-conversation under `data/conversations/<id>.json`
(indexed in `data/conversations/index.json`), plus optional Obsidian vault integration
(`OBSIDIAN_VAULT` env var) for surfacing a TODO-list and recent vault files in the UI. Messages are
queued during the async init window so no client message is dropped mid-handshake (see the
`messageQueue` pattern in the `connection` handler). Confirmation prompts round-trip as
`{type: 'confirmation'}` / `{type: 'confirm', decision}` WS messages resolved against
`pendingConfirmations` on the client record.

**Frontend** (`ui/`): Vite + React 19 + Tailwind 4, talks to the WS server via `useWebSocket` hook
(hardcoded to `ws://<host>:3001`, no path prefix despite the vite proxy config using `/ws` — the proxy
is currently unused since the hook connects directly). State (history, memories, conversations, vault
files, notices, config) is pushed from the server on connect and after most mutations rather than
polled.

## Working across the two npm projects

The root `package.json` and `ui/package.json` are independent (separate lockfiles, separate
`node_modules`). Changes touching both the WS message schema (`ws-server.ts` message `type`s) and the
UI (`ui/src/types/index.ts` `WsMessage`, `useWebSocket` handlers) need to be kept in sync manually —
there's no shared types package between backend and frontend.
