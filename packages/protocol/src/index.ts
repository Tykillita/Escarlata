...
export const clientMessageSchema = z.discriminatedUnion('type', [
  ...
  z.object({ type: z.literal('get_state') }), z.object({ type: z.literal('get_audit'), count: z.number().int().min(1).max(200).optional() }),
  z.object({ type: z.literal('get_memories') }), z.object({ type: z.literal('get_notices') }), z.object({ type: z.literal('get_vault_files') }),
  z.object({ type: z.literal('get_directives') }), z.object({ type: z.literal('get_ollama_models') }), z.object({ type: z.literal('list_chats') }),
  z.object({ type: z.literal('new_chat') }), z.object({ type: z.literal('clear_history') }), z.object({ type: z.literal('abort') }), z.object({ type: z.literal('ping') }),
  ...
  z.object({ type: z.literal('push_subscribe'), subscription: z.object({ endpoint: z.string().url().max(4_096) }).passthrough() }),
  z.object({ type: z.literal('push_unsubscribe'), endpoint: z.string().url().max(4_096) }),
